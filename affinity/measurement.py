...
def dynamic_affinity(vnf_a, vnf_b, fg):
    ...
    y = 0
    if (fg is not None):
        flow = next((x for x in fg.flows if ((x.src == vnf_a.id and x.dst == vnf_b.id) or (
            x.src == vnf_b.id and x.dst == vnf_a.id))), None)
...
def affinity_measurement(vnf_a, vnf_b, fg):
    affinity_result = {}
    if (fg is not None):
        affinity_result['fg'] = fg.id
        affinity_result['vnf_a'] = vnf_a.id
        affinity_result['vnf_b'] = vnf_b.id
        ...
        if (len(common_fgs) != 0):
            for fg in common_fgs:
                affinity_result[fg.id] = total_affinity(vnf_a, vnf_b, fg)
        else:
